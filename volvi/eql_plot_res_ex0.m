function eql_plot_res_ex0(varargin)
    %% *SET-UP*
    inp = varargin{1};
    out = varargin{2};
    ...
    %% *PLOT INPUT MOTION*
    data = importdata('thv_volvi_nl_weak_bedrock.txt');
    nln.vtm.bdr = data(:,1);
    nln.thv.bdr = data(:,2);
    ...
    fpplot('xpl',{nln.vtm.bdr;out.vtm},'ypl',{nln.thv.bdr;out.thv(end,:)},...
        ...
        'xlm',{xlm.thv},'xtk',{xtk.thv},'ylm',{[-0.006;0.006]},'ytk',{-0.006:0.002:0.006},...
        'tit',{'G.L. -300m - Weak Motion'},...
        'xlb',{'t [s]'},'ylb',{'v(t) [m/s]'});
    % fourier's spectra
    fpplot('xpl',{nln.vfr.bdr;out.vfr},'ypl',{nln.fsv.bdr;out.fsv(end,:)},...
        ...
        'tit',{'G.L. -300m - Weak Motion'},'scl',{'log'},...
        'xlb',{'f [Hz]'},'ylb',{'FSV [m]'});
    ...
    fpplot('xpl',{nln.bsr(1,:);out.bsr(1,:)},'ypl',{abs(nln.bsr(2,:));abs(out.bsr(2,:))},...
        ...
        'tit',{'G.L. 0m/G.L. -300m - Weak Motion'},'scl',{'log'},...
        'xlb',{'f [Hz]'},'ylb',{'BHSR [1]'});
    return
end
